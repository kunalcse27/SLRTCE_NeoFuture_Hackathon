...
export default function TeacherPage() {
  ...
  const getFormatIcon = (format) => {
    switch (format) {
      ...
      case "JPG":
        return { icon: "image", color: "text-blue-400", bg: "bg-blue-500/10" };
      case "Text":
        return {
          icon: "description",
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        ...
        >
          ...
          <p className="text-on-surface-variant max-w-md font-label text-base">
            Create and manage academic tasks in multiple formats.
          </p>
        </motion.div>
        ...
        <div className="lg:col-span-1">
          <section className="glass-panel p-8 rounded-[3rem] border border-outline-variant/15 flex flex-col gap-8 h-full">
            ...
            <form onSubmit={addTask} className="relative group">
              ...
              <button
                type="submit"
                className="absolute right-3 top-1/2 -translate-y-1/2 w-12 h-12 rounded-2xl bg-secondary text-on-secondary flex items-center justify-center shadow-lg hover:scale-110 transition-transform active:scale-95"
              >
                <span
                  className="material-symbols-outlined font-bold"
                ...
                {tasks.map((task) => (
                  ...
                  >
                    ...
                    >
                      {task.completed && (
                        <span
                          className="material-symbols-outlined text-xs text-on-secondary font-black"
                          data-icon="done"
                        >
                          done
                        </span>
                    ...
                    <span
                      className={`flex-grow text-sm font-bold ${task.completed ? "line-through text-slate-500" : "text-on-surface"}`}
                    >
                      {task.text}
                    </span>
                  </motion.div>
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-12 h-12 rounded-2xl ${formatInfo.bg} flex items-center justify-center ${formatInfo.color}`}
                          >
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-outline-variant/10">
                        ...
                        <span
                          className={`text-[10px] font-black px-3 py-1 rounded-full ${formatInfo.bg} ${formatInfo.color} border border-current/20`}
                        >
                          {assignment.format}
                        </span>
                      </div>
            ...
            >
              <div className="flex justify-between items-start mb-10">
                <div className="space-y-2">
                  <h3 className="text-3xl font-black text-on-surface font-headline tracking-tighter">
                    Create New Task
                  </h3>
                  <p className="text-sm text-on-surface-variant font-bold uppercase tracking-widest leading-none">
                    Format: {newAssignment.format}
                  </p>
                </div>
                <button
                  onClick={() => setShowAssignmentModal(false)}
              ...
              <form onSubmit={createAssignment} className="space-y-8">
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.3em] text-secondary ml-1">
                    Task Title
                  </label>
                  <input
                    type="text"
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    {formats.map((f) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setNewAssignment({ ...newAssignment, format: f })
                        }
        ...
        {notification && (
          ...
          >
            <div className="w-10 h-10 rounded-2xl bg-secondary flex items-center justify-center shadow-lg">
              <span
                className="material-symbols-outlined font-black text-on-secondary"
                data-icon="done_all"
              >
                done_all
              </span>
